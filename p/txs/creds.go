...
// Credential wire + the split point of a signed tx. A signed tx is
// unsigned_buffer ‖ creds_buffer, both self-delimiting zap buffers, so the
// unsigned bytes are a genuine byte-prefix of the signed bytes and the
// unsigned buffer's own size field is where a reader splits them.
...
func parseCredsBuf(b []byte) ([]verify.Verifiable, error) {
	...
	for i := 0; i < n; i++ {
		...
		// come off the wire. start <= total makes total-start safe.
...
// concat returns a ‖ b as a fresh slice. Never append into the unsigned
// buffer: its backing array is the zap builder's and its size field at
// [12:16] must keep naming the unsigned length.
