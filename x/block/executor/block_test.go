...
import (
	...
	"github.com/luxfi/proto/x/metrics/metricsmock"
	"github.com/luxfi/proto/x/state/statemock"
	"github.com/luxfi/proto/x/txs"
	txexecutor "github.com/luxfi/proto/x/txs/executor"
	"github.com/luxfi/proto/x/txs/mempool"
	...
	tests := []test{
		{
			...
			blockFunc: func(ctrl *gomock.Controller) *Block {
				...
				validTx.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
				syntacticallyInvalidTx := &txs.Tx{Unsigned: unsignedSyntacticallyInvalidTx}
				syntacticallyInvalidTx.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
				semanticallyInvalidTx := &txs.Tx{Unsigned: unsignedSemanticallyInvalidTx}
				semanticallyInvalidTx.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
				executionFailsTx := &txs.Tx{Unsigned: unsignedExecutionFailsTx}
				executionFailsTx.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
		...
		},
		{
			...
			blockFunc: func(ctrl *gomock.Controller) *Block {
				...
				tx1.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
				tx2 := &txs.Tx{Unsigned: unsignedTx2}
				tx2.SetBytes(crypto.RandomBytes(16), crypto.RandomBytes(16))
...
func defaultTestBackend(bootstrapped bool, sharedMemory atomic.SharedMemory) *txexecutor.Backend {
	...
	if sharedMemory != nil {
		backend.SharedMemory = sharedMemory
	}
	return backend
}
