...
// caller-supplied descriptor, and owns the codec those bytes are
// written with. A caller (e.g. a primary-network genesis builder)
// describes the primary asset and gets bytes back; the asset shape and
// its serialization sit together here.
//
...
// because NewGenesis sorts both the asset set and each asset's
...
func BuildBytes(
	...
	for _, h := range holders {
		def.InitialState.FixedCap = append(def.InitialState.FixedCap, GenesisHolder{
			Amount:  h.Amount,
			Address: h.Address,
		})
	}

	g, err := NewGenesis(networkID, map[string]GenesisAssetDefinition{
		asset.Symbol: def,
	})
	if err != nil {
		return nil, err
...
func AssetIDFromBytes(genesisBytes []byte) (ids.ID, error) {
	return AssetIDFromGenesisBytes(genesisBytes)
}
