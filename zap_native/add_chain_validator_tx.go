...
// AddChainValidatorTx — chain validator add. POST-rebrand: rebranded
// from the legacy "AddSubnetValidatorTx" name in upstream Lux v0.x. The
// txs package no longer uses that legacy identifier in *.go names; our
// v3 form pins ChainValidator semantics. ChainAuth (credential proof)
